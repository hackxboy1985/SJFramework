...
-(NSString *)formattedTime{
    ...
    if (!hasAMPM) { //24小时制
        if (hour <= 24 && hour >= 0) {
            ...
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:NSLocalizedString(@"M-d HH:mm", @"")];
        }else {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"yyyy-MM-dd HH:mm"];
        }
    }else {
        if (hour >= 0 && hour <= 6) {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:NSLocalizedString(@"aa hh:mm", @"")];
