...
- (void)replaceChild:(GDataXMLNode *)old cur:(GDataXMLNode*)cur{
    assert([old kind] != GDataXMLAttributeKind && [cur kind] != GDataXMLAttributeKind);    
    
    if (xmlNode_!=NULL && cur != NULL && old!=NULL) {
        
        [self releaseCachedValues];
             
        xmlNodePtr oldNode = [old XMLNode];
        xmlNodePtr curNodeCopy = [cur XMLNodeCopy];
        if (curNodeCopy) {
            
            xmlNodePtr resultNode = xmlReplaceNode(oldNode, curNodeCopy);
            if (resultNode == NULL) {
    ...
    if (xmlNode_ != NULL) {
        ...
        for (GDataXMLNode *child in children) {
            ...
            if (currNode->type == XML_ELEMENT_NODE) {
                ...
                // if ([qName isEqual:name]) { //delete by weizhiqiang
                if(NSOrderedSame==[qName compare:name options:NSCaseInsensitiveSearch]){//add by weizhiqiang                    
                    if (array == nil) {
...
#pragma mark up or lower CaseInsensitive attributeForName add by sj.
- (GDataXMLNode *)attributeForNameCaseInsensitive:(NSString *)name {
    ...
    if (xmlNode_ != NULL) {
        ...
        if (attrPtr == NULL) {
            ...
            for (GDataXMLNode *attr in attributes) {
                ...
                if (NSOrderedSame == [[attr name] compare:name options:NSCaseInsensitiveSearch]) {
                    return attr;
                }
            }
        }
