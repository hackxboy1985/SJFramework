...
// 注意:此只适合开发调试，参数配置和参数加密需要放到服务器端处理
// 服务器端Demo请查看包的文件
// 更新时间：2015年3月3日
// 负责人：李启波（marcyli）
//============================================================
- ( NSMutableDictionary *)sendPay_wx:(NSString*)orderNo orderDesc:(NSString*)orderDesc price:(float)price notifyUrl:(NSString*)notifyUrl
{

    //订单标题，展示给用户
    NSString *order_name    = orderDesc.length?orderDesc:@"支付";
    //订单金额,单位（分）
    NSString *order_price   = [NSString stringWithFormat:@"%d",(int)price];//@"1";//1分钱测试
    ...
    NSString *orderno   = orderNo.length?orderNo:[NSString stringWithFormat:@"%ld",time(0)];
